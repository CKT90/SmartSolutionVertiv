    ...
    class UserNumberGenerator < Module
      BASE           = 10
      DEFAULT_LENGTH = 9
      ...
      def included(host)
        ...
        host.class_eval do
          validates(:id, presence: true, uniqueness: { allow_blank: true })

          before_validation do |instance|
            instance.id ||= generator_method.call(host)
          end

          define_singleton_method(:user_number_generator) { generator_instance }
        end
      end
      ...
      def generate_permalink(host)
        ...
        loop do
          ...
          return candidate unless host.exists?(id: candidate)
